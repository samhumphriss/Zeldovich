...
def getXi(density, nrbins=40, boxsize=512., get2d=False, deconvolve_cic=False, exp_smooth=0.0):
    ...
    #Fourier transform of density
    density=N.fft.rfftn(density)
    sk=density.shape
    
    #Square the density in Fourier space to get the 3D power, make sure k=0 mode is 0
    density=(density*N.conjugate(density)).astype(N.float)
    density[0,0,0]=0.0
    ...
    if (deconvolve_cic):
        ...
        ww=N.sin(kmin*a*N.pi/(2*kny))**4/(kmin*a*N.pi/(2*kny))**4*N.sin(kmin*b*N.pi/(2*kny))**4/(kmin*b*N.pi/(2*kny))**4*N.sin(kmin*c*N.pi/(2*kny))**4/(kmin*c*N.pi/(2*kny))**4
	ww[N.where((a==0) | (b==0) | (c==0))]=1.0

        density=density/ww
        
    if (exp_smooth!=0):
        dk2=dk2/N.exp(-kgrid**2*exp_smooth**2)
        
    #Fourier transform back to config space
    density = N.fft.irfftn(density)/ngrid**3
    ...
    if (get2d):
        ...
        for i in N.arange(ngrid/2):
            ...
            xi3d_sort = density[i,:,:].flatten()[index]
        ...
        for i in N.arange(0, nrbins-1):
            if (cuts[i+1]>cuts[i]):
                numinbin[i]=N.sum(c0[cuts[i]:cuts[i+1]])
                xir[i]=N.sum(c0[cuts[i]:cuts[i+1]]*density[cuts[i]:cuts[i+1]])
